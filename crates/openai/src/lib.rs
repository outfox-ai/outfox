...
//!
//! To use custom types for inputs and outputs, enable `byot` feature which provides additional
//! generic methods with same name and `_byot` suffix. This feature is available on methods whose
//! return type is not `Bytes`
//!
//! ```
//! # #[cfg(feature = "byot")]
//! # tokio_test::block_on(async {
...
//! # #[cfg(feature = "byot")]
//! # tokio_test::block_on(async {
//! # use async_openai::{Client, spec::responses::{CreateResponse, Response}};
//! # let client = Client::new();
...
//!
//! These granular types are enabled when the corresponding API feature is enabled - for example
//! `responses` will enable `response-types`.
//!
//! ## WASM
...
//! **Individual Request**
//!
//! Certain individual APIs that need additional query or header parameters - these can be provided
//! by chaining `.query()`, `.header()`, `.headers()` on the API group.
//!
//! For example:
//! ```
//! # tokio_test::block_on(async {
...
//! **All Requests**
//!
//! Use `Config`, `OpenAIConfig` etc. for configuring url, headers or query parameters globally for
//! all requests.
//!
//! ## OpenAI-compatible Providers
//!
//! Even though the scope of the crate is official OpenAI APIs, it is very configurable to work with
//! compatible providers.
//!
//! **Configurable Path**
//!
//! In addition to `.query()`, `.header()`, `.headers()` a path for individual request can be
//! changed by using `.path()`, method on the API group.
//!
//! For example:
//! ```
//! # tokio_test::block_on(async {
//! # use async_openai::{Client, spec::chat::CreateChatCompletionRequestArgs};
//! # use async_openai::traits::RequestOptionsBuilder;
//! # let client = Client::new();
...
//! #     .build()
//! #     .unwrap();
//! client.chat().path("/v1/messages")?.create(request).await?;
//! # Ok::<(), Box<dyn std::error::Error>>(())
//! # });
//! ```
//!
//! **Dynamic Dispatch**
//!
//! This allows you to use same code (say a `fn`) to call APIs on different OpenAI-compatible
//! providers.
//!
//! For any struct that implements `Config` trait, wrap it in a smart pointer and cast the pointer
//! to `dyn Config` trait object, then create a client with `Box` or `Arc` wrapped configuration.
//!
//! For example:
//! ```
//! use async_openai::Client;
