...
use crate::error::{ApiError, OpenAIError, WrappedError, map_deserialization_error};
#[cfg(feature = "file")]
        ...
        O: DeserializeOwned + std::marker::Send + 'static,
    {
        ...
        tokio::spawn(async move {
            ...
            while let Some(event_result) = event_stream.next().await {
                match event_result {
                    Err(e) => {
                        if let Err(_e) = tx.send(Err(OpenAIError::Stream(
                            StreamError::EventStream(e.to_string()).to_string(),
                        ))) {
                            break;
                        }
                    }
                    ...
                    match e {
                        OpenAIError::ApiError(api_error) => {
                            if status.is_server_error() {
                                ...
                                && api_error.kind != Some("insufficient_quota".to_string())
                            {
                                // Rate limited retry...
...
async fn read_response(response: Response) -> Result<(Bytes, HeaderMap), OpenAIError> {
    ...
    if status.is_server_error() {
        // OpenAI does not guarantee server errors are returned as JSON so we cannot deserialize
        // them.
...
async fn map_stream_error(value: EventSourceError) -> OpenAIError {
    match value {
        ...
        _ => OpenAIError::Stream(StreamError::ReqwestEventSource(value).to_string()),
    }
}
