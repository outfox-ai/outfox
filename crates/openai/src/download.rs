...
pub(crate) async fn download_url<P: AsRef<Path>>(
    ...
    let parsed_url = Url::parse(url).map_err(|e| OpenAIError::FileSave(e.to_string()))?;
    let response = reqwest::get(url)
        .await
        .map_err(|e| OpenAIError::FileSave(e.to_string()))?;

    if !response.status().is_success() {
        return Err(OpenAIError::FileSave(format!(
    ...
    tokio::fs::create_dir_all(dir.as_path())
        .await
        .map_err(|e| OpenAIError::FileSave(format!("{}, dir: {}", e, dir.display())))?;

    tokio::fs::write(
        file_path.as_path(),
        response.bytes().await.map_err(|e| {
            OpenAIError::FileSave(format!("{}, file path: {}", e, file_path.display()))
        })?,
    )
    .await
    .map_err(|e| OpenAIError::FileSave(e.to_string()))?;
...
pub(crate) async fn save_b64<P: AsRef<Path>>(b64: &str, dir: P) -> Result<PathBuf, OpenAIError> {
    ...
    .await
    .map_err(|e| OpenAIError::FileSave(format!("{}, path: {}", e, path.display())))?;
