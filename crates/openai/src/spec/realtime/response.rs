...
use crate::spec::realtime::session::{RealtimeVoice, ToolChoice};
use crate::spec::realtime::{
    MaxOutputTokens, RealtimeAudioFormats, RealtimeConversationItem, RealtimeTool,
};
use crate::spec::responses::Prompt;
...
pub struct RealtimeResponseUsage {
    ...
    /// tokens here are counted as a subset of input tokens, meaning input tokens will include
...
pub struct RealtimeResponseStatusDetail {
    ...
    /// `max_output_tokens` or `content_filter` (the  server-side safety filter activated and
    ...
    pub kind: RealtimeResponseStatusDetailType,
}
