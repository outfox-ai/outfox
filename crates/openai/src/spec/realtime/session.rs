...
use crate::spec::mcp::MCPTool;
use crate::spec::responses::{Prompt, ToolChoiceFunction, ToolChoiceMCP, ToolChoiceOptions};
...
pub enum RealtimeTurnDetection {
    ...
    ServerVAD {
        ...
        /// `interrupt_response` is set to `false` this may fail to create a response if the model
        ...
        /// Response)  will be emitted when the timeout is reached. Idle timeout is
        ...
        /// occurs. If `true` then the response will be cancelled, otherwise it will
    ...
    SemanticVAD {
        ...
        /// quickly. `auto` is the default and is equivalent to `medium`. `low`, `medium`, and
        /// `high` have max timeouts of 8s, 4s, and 2s respectively.
        eagerness: String,
...
pub struct AudioInput {
    ...
    /// consumes audio directly. Transcription runs asynchronously through [the /audio/transcriptions endpoint](https://platform.openai.com/docs/api-reference/audio/createTranscription)
...
pub struct AudioOutput {
    ...
    /// `alloy`, `ash`, `ballad`, `coral`, `echo`, `sage`, `shimmer`, `verse`, `marin`, and
...
pub enum Session {
    ...
    // the largest variant contains at least 600 bytes, the second-largest variant contains at
    // least 144 bytes
...
pub struct RealtimeSession {
    ...
    /// `["audio"]`, indicating that the model will respond with audio plus a transcript.
    /// `["text"]` can be used to make the model respond with text only. It is not possible to
    ...
    /// Set to null to disable tracing. Once tracing is enabled for a session, the configuration
    /// cannot be modified.
    ///
    ...
    /// control token usage and cost. Truncation will reduce the number of cached tokens on the
    /// next  turn (busting the cache), since messages are dropped from the beginning of the
    ...
    /// server will never truncate but would instead return an error if the conversation
