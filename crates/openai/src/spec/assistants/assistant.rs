...
/// `auto` ranker and a score_threshold of 0.
///
...
pub struct FileSearchRankingOptions {
    ...
    /// The score threshold for the file search. All values must be a floating point number between
    /// 0 and 1.
...
/// `auto` is the default value and means the model can pick between generating a message or calling
/// one or more tools. `required` means the model must call one or more tools before responding to
/// the user. Specifying a particular tool like `{"type": "file_search"}` or `{"type": "function",
