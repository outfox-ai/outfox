...
use crate::spec::chat::{ChatCompletionTool, ImageDetail, InputAudio, ResponseFormat};
use crate::spec::graders::{
    GraderLabelModel, GraderPython, GraderScoreModel, GraderStringCheck, GraderTextSimilarity,
};
// Re-export commonly used types
pub use crate::spec::responses::{EasyInputMessage, InputTextContent, ReasoningEffort};
use crate::spec::responses::{ResponseTextParam, Tool};
...
/// Supports text, output text, input images, and input audio, either as a single item or an array
/// of items.
#[derive(Debug, Deserialize, Serialize, Clone, PartialEq)]
#[serde(untagged)]
...
pub struct CreateEvalCustomDataSourceConfig {
    ...
    /// Whether the eval should expect you to populate the sample namespace (ie, by generating
    /// responses off of your data source).
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub struct CreateEvalCompletionsRunDataSource {
    ...
    /// model. Can either be a reference to a prebuilt trajectory (ie,
...
pub struct EvalResponsesSource {
    ...
    /// Only include items created after this timestamp (inclusive). This is a query parameter used
    /// to select responses.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub created_after: Option<u64>,
    /// Only include items created before this timestamp (inclusive). This is a query parameter
