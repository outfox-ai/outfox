...
use crate::spec::images::ImageInput;
...
pub struct CreateImageRequest {
    ...
    /// The model to use for image generation. One of `dall-e-2`, `dall-e-3`, or the GPT image
    /// model (`gpt-image-1`, `gpt-image-1-mini`, `gpt-image-1.5`). Defaults
    ...
    /// generated. This parameter isn't supported for the GPT image models which will always
    ...
    /// one of `vivid` or `natural`. Vivid causes the model to lean towards generating
...
pub struct CreateImageEditRequest {
    ...
    ///
    /// For the GPT image models (the GPT image models, `gpt-image-1-mini`, and `gpt-image-1.5`),
    /// each image should be a `png`, `webp`, or `jpg` file less
    ...
    /// `image` should be edited. If there are multiple images provided, the mask will be applied
    /// on the first image. Must be a valid PNG file, less than 4MB, and have the same
    /// dimensions as `image`.
    ...
    /// URLs are only valid for 60 minutes after the image has been generated. This parameter
    /// is only supported for `dall-e-2`, as the GPT image models will always return
    ...
    /// facial features, of input images. This parameter is only supported for the GPT image
    /// models. Unsupported for `gpt-image-1-mini`. Supports `high` and `low`. Defaults to
    /// `low`.
