...
fn default_api_key() -> String {
    #[cfg(not(target_family = "wasm"))]
    {
        std::env::var("OPENAI_API_KEY")
            .or_else(|_| {
                std::env::var("OPENAI_ADMIN_KEY").inspect(|_| {
...
impl OpenAIConfig {
    /// Create client with default [OPENAI_API_BASE] url (can also be changed with OPENAI_BASE_URL
    /// env var) and default API key from OPENAI_API_KEY env var
    pub fn new() -> Self {
        Default::default()
    }
